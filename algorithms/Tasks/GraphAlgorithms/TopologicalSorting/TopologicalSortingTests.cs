...
{
    [Fact]
    public void DfsSort_ShouldBeOk( )
    {
        // Arrange
    ...
    [Fact]
    public void KahnSort_ShouldBeOk( )
    {
        // Arrange
        var node1 = new TopologicalSorting.Vertex( 1 );
        var node2 = new TopologicalSorting.Vertex( 2 );
        ...
        // Assert
        var res = TopologicalSorting.KahnSort( graph );
        Assert.Equal( 6, res.Count );
        var values = res.Select( c => c.Value ).ToList( );
        Assert.Equal( values, [1, 2, 4, 3, 5, 6] );
    }
    
    [Fact]
    public void KahnSort_WithCycle_ShouldThrow( )
    {
        // Arrange
        var node1 = new TopologicalSorting.Vertex( 1 );
        var node2 = new TopologicalSorting.Vertex( 2 );
        ...
        // Assert
        Assert.Throws<InvalidOperationException>( () => TopologicalSorting.KahnSort( graph )  );
    }
}